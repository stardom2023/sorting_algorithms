...
void mergesort(int *array, int *tmp, int start, int end)
{
	...
	if (tan >= start)
	{
		mergesort(array, tmp, start, tan);
		mergesort(array, tmp, tan + 1, end);
		merge(array, tmp, start, tan, end);
	}
}
/**
